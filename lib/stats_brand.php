...
class stats_brand
{
    ...
    private function get_sql()
    {
        ...
        if ($this->start_date != '' && $this->end_date != '') {
            $result = $sql->setQuery('SELECT brand, COUNT(brand) as "count" FROM ' . rex::getTable('pagestats_dump') . ' where date between :start and :end GROUP BY brand ORDER BY count DESC', ['start' => $this->start_date, 'end' => $this->end_date]);
        } else {
    ...
    public function get_list()
    {
        ...
        if ($this->start_date != '' && $this->end_date != '') {
            $list = rex_list::factory('SELECT brand, COUNT(brand) as "count" FROM ' . rex::getTable('pagestats_dump') . ' where date between "' . $this->start_date . '" and "' . $this->end_date . '" GROUP BY brand ORDER BY count DESC');
        } else {
